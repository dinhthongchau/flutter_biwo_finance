...
mixin CalendarScreenMixin {

  Widget buildBody(CalendarState state, BuildContext context) {
    return Padding(
  ...
  PreferredSizeWidget buildHeaderCalendar(BuildContext context) {
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                ...
                hasTransactions
                    ? () => context.read<CalendarBloc>().add(
                  SelectDateCalendarEvent(dayIndex),
                )
                    : null,
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 200),
      ...
      children: [
        Expanded(
          child: GestureDetector(
            onTap:
                () => context.read<CalendarBloc>().add(
              const ChangeChartTypeEvent(ChartTypeCalendar.spends),
            ),
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 15),
              decoration: BoxDecoration(
                color:
                state.selectedChartType == ChartTypeCalendar.spends
                    ? AppColors.caribbeanGreen
        ...
        Expanded(
          child: GestureDetector(
            onTap:
                () => context.read<CalendarBloc>().add(
              const ChangeChartTypeEvent(ChartTypeCalendar.categories),
            ),
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 15),
              decoration: BoxDecoration(
                color:
                state.selectedChartType == ChartTypeCalendar.categories
                    ? AppColors.caribbeanGreen
                    : AppColors.lightGreen,
                borderRadius: BorderRadius.circular(25),
              ),
      ...
      final selectedDateTransactions =
          state.allTransactions
              ?.where(
                (t) =>
            state.showAllMonth
                ? (t.time.year == state.currentMonth.year &&
                t.time.month == state.currentMonth.month)
      ...
      child: SfCircularChart(
        series: <PieSeries<PieChartData, String>>[
          PieSeries<PieChartData, String>(
            ...
            dataLabelMapper:
                (PieChartData data, _) => '${data.y.toStringAsFixed(1)}%',
          ),
        ],
      ),
    );
