...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  bool _showAllMonth = true; // Default to show all month transactions

  List<ChartSampleData> _chartData = [];
  static const int maxCategories = 5; // Limit categories for better readability

  @override
  void initState() {
    super.initState();
    // Trigger transaction loading via BLoC
  ...
  void _generateSpendsData(List<TransactionModel> transactions) {
    ...
    transactions.where((t) => t.idCategory.moneyType == MoneyType.expense).toList();
    ...
    _chartData = _createOptimizedChartData(categoryTotals, totalExpenses, _getExpenseColors());
    setState(() {});
  }
  ...
  void _generateCategoriesData(List<TransactionModel> transactions) {
    ...
    _chartData = _createOptimizedChartData(categoryTotals, totalAmount, _getCategoryColors());
    setState(() {});
  }
  ...
  // Improved chart data creation with category grouping
  List<ChartSampleData> _createOptimizedChartData(
      Map<String, double> categoryTotals, double total, List<Color> colors) {
    if (categoryTotals.isEmpty) {
      return [ChartSampleData(x: 'No Data', y: 100, color: AppColors.lightBlue)];
    }
    ...
    // Group remaining small categories as "Others"
    if (sortedEntries.length > maxCategories - 1) {
      ...
      // Also include small categories from the first group
      for (int i = 0; i < sortedEntries.length && i < maxCategories - 1; i++) {
        final entry = sortedEntries[i];
        double percentage = total > 0 ? (entry.value / total) * 100 : 0;
        if (percentage <= 2.0) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: const Text(
            'Select Month',
            style: TextStyle(color: AppColors.fenceGreen, fontWeight: FontWeight.bold),
                  ...
                  child: Padding(
                    ...
                    child: Center(
                      child: Text(
                        ...
                        style: const TextStyle(color: AppColors.fenceGreen, fontSize: 16),
                      ),
                    ),
                  ),
                );
  ...
  void _changeYear() {
    ...
    if (state is TransactionSuccess) {
      showModalBottomSheet(
        context: context,
        builder: (context) {
          final yearsWithData = state.allTransactions.map((t) => t.time.year).toSet().toList();
          return Container(
            height: 200,
                    ...
                    child: state is TransactionLoading
                      ...
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 20),
                          if (_selectedChartType == ChartType.categories && _chartData.isNotEmpty)
                            Column(
                              children: [
                                _buildLegend(),
                                const SizedBox(height: 20),
  ...
  Widget _buildCalendar(TransactionState state) {
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        _showAllMonth ? Icons.calendar_month : Icons.calendar_today,
                        color: _showAllMonth
                            ? AppColors.honeydew
                            : AppColors.fenceGreen,
                        size: 16,
    ...
    padding: EdgeInsets.zero,
            ...
            itemBuilder: (context, index) {
              ...
              final isSelected = !_showAllMonth &&
                  dayDate.day == _selectedDate.day &&
                  dayDate.month == _selectedDate.month &&
                  dayDate.year == _selectedDate.year;
              final hasTransactions = state is TransactionSuccess &&
                  state.allTransactions.any((transaction) =>
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: hasTransactions && !isSelected
                        ? Border.all(color: AppColors.caribbeanGreen.withValues(alpha: 0.3), width: 1)
                        : null,
        ...
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _selectedChartType == ChartType.spends
                    ? AppColors.caribbeanGreen
                    : AppColors.lightGreen,
        ...
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 15),
              decoration: BoxDecoration(
                color: _selectedChartType == ChartType.categories
                    ? AppColors.caribbeanGreen
                    : AppColors.lightGreen,
                borderRadius: BorderRadius.circular(25),
              ),
  ...
  // Improved legend with horizontal scrolling for better space usage
  Widget _buildLegend() {
    return SizedBox(
      height: 40,
